...
class AttributeController extends Controller
{
    public function index()
    {
        ...
        if (request()->ajax()) {
            return datatables()->of(attribute::latest()->get())
    ...
    public function form(Request $request, $form_choice = "",$id="")
    {
        if ($request->input()) {
            ...
            $record = ($form_choice == 'edit') ? attribute::find($id) : new attribute ;
            ...
            if (!empty($request->file('image'))) {
                if ($form_choice == 'edit') {
                    $image_path = attribute::where('id',$id)->first()->image;
            ...
            if($form_choice == 'edit' || $form_choice == 'duplicate'){
                $content['record'] = attribute::findOrFail($id);
            }
    ...
    public function delete_all(Request $request)
    {
        if ($request->input('checkbox_value')) {
            ...
            for ($i=0; $i < count($id); $i++) {
                $data = attribute::findorFail($id[$i]);
                $data->delete();
            }
